	...
	class PropertiesPanel {
		...
		buildStatisticsView() {
			...
			const categoryList = Object.entries(categoryCounts)
				...
				.map(
					([cat, count]) =>
						`<li class="stats-list-item"><span class="stats-list-label">${this.escapeHtml(cat)}</span><span class="stats-list-value">${count}</span></li>`,
				)
				.join('');
			this.content.innerHTML = `
		...
		buildLoopProperties(block) {
			...
			this.content.innerHTML = `
                ...
                <div class="property-section">
                    ...
                    <input type="number" id="prop-loop-count" class="property-input" value="${this.escapeAttribute(block.loopCount || 3)}" min="1" step="1">
                </div>
                <div class="property-section">
                    <label class="property-label">每次循环间隔 (毫秒)</label>
                    <input type="number" id="prop-loop-delay" class="property-input" value="${this.escapeAttribute(block.loopDelay || 0)}" min="0" step="100">
		...
		buildVariableProperties(block) {
			...
			this.content.innerHTML = `
                ...
                <div class="property-section">
                    ...
                    <input type="text" id="prop-var-name" class="property-input" value="${this.escapeAttribute(block.varName || '')}" placeholder="例如：myVar">
                </div>
                <div class="property-section">
		...
		buildFunctionProperties(block) {
			...
			this.content.innerHTML = `
                ${outputMappingsHtml}
                <div class="property-section"><label class="property-label">函数名称</label><input type="text" id="prop-func-name" class="property-input" value="${this.escapeAttribute(block.title || '')}" placeholder="例如：处理数据"></div>
                <div class="property-section"><label class="property-label">函数描述</label><input type="text" id="prop-func-desc" class="property-input" value="${this.escapeAttribute(block.description || '')}" placeholder="简短描述"></div>
                ${inputFieldsHtml}
                <div class="property-section"><label class="property-label">输入参数 (逗号分隔)</label><input type="text" id="prop-func-inputs" class="property-input" value="${this.escapeAttribute(params.join(', '))}" placeholder="例如：input1, input2"></div>
                <div class="property-section"><label class="property-label">输出参数</label><input type="text" id="prop-func-output" class="property-input" value="${this.escapeAttribute(outputName)}" placeholder="例如：result"></div>
                <div class="property-section"><label class="property-label">代码</label><textarea id="prop-func-code" class="property-textarea">${this.escapeHtml(block.code)}</textarea></div>
		...
		buildCodeProperties(block) {
			...
			this.content.innerHTML = `
                ${outputMappingsHtml}
                ${inputFieldsHtml}
                <div class="property-section"><label class="property-label">代码</label><textarea id="prop-code" class="property-textarea" placeholder="在此编写 JavaScript 代码...">${this.escapeHtml(block.code)}</textarea></div>
			...
			try {
				...
				if (input === '') value = null;
					...
					try {
						...
						eda.sys_Message.showToastMessage('无效的JSON格式: ' + err.message, 'info', 1);
						value = input;
					}
				} else value = input;
			...
			this.content.innerHTML = `
                <div class="property-section">
                    ...
                    <textarea id="prop-annotation-text" class="property-textarea"
                              placeholder="输入注释内容...">${this.escapeHtml(block.annotationText || '')}</textarea>
                </div>
                <div class="property-actions">
                    <button class="property-button secondary" id="prop-cancel">取消</button>
			...
			block.inputs.forEach((input, index) => {
				...
				if (connection) {
					...
					if (sourceBlock) {
						...
						const displayPath = connection.fromPath && connection.fromPath !== '$' ? connection.fromPath.replace(/^\$/, '') : '';
				...
				html += `
                    <div class="input-field-group">
                        <label class="property-label">${this.escapeHtml(inputName)}</label>
                        <input type="text"
                               class="property-input input-field"
                               data-block-id="${block.id}"
                               data-port-index="${index}"
                               value="${this.escapeAttribute(currentValue)}"
                               placeholder="${this.escapeAttribute((typeof input === 'object' && input.description) || '输入值或粘贴变量引用')}">
			...
			if (block.type === 'variable') {
				const items = outgoing
					.map((conn, index) => {
						...
						const targetInput =
							targetBlock && targetBlock.inputs[conn.toPort]
								? window.WorkflowApp.Helpers.portName(targetBlock.inputs[conn.toPort])
								: `input${conn.toPort}`;
                        ...
                        <div class="mapping-row" style="position: relative;">
                            <div class="mapping-label"><span class="mapping-source-link" data-source-id="${block.id}" style="color: #66d9ef; cursor: pointer;">${this.escapeHtml(outputName)}${this.escapeHtml(displayPath)}</span> → <span class="mapping-target-link" data-target-id="${conn.toId}" style="color: #66d9ef; cursor: pointer;">${this.escapeHtml(targetName)}.${this.escapeHtml(targetInput)}</span></div>
                            <div style="display: flex; gap: 4px; align-items: center;">
                                <div class="property-input mapping-path-select"
                                     ...
                                     title="${this.escapeAttribute(displayPath || '点击选择路径')}">${displayPath ? `<span style="color: #a6e22e;">${this.escapeHtml(displayPath)}</span> = ${this.escapeHtml(displayValue)}` : '<span style="color: #75715e;">点击选择路径...</span>'}</div>
                    ...
                    <div class="mapping-row" style="position: relative;">
                        ...
                        <div style="display: flex; gap: 4px; align-items: center;">
                            <div class="property-input mapping-path-select"
                                 ...
                                 data-to-port="${conn.toPort}"
                                 style="flex: 1; cursor: pointer; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; line-height: 36px;"
                                 title="${this.escapeAttribute(displayPath || '点击选择路径')}">${displayPath ? this.escapeHtml(displayPath) : '<span style="color: #75715e;">点击选择路径...</span>'}</div>
                            ${hasPath ? `<button class="output-clear-btn" title="清除映射路径" data-from-id="${conn.fromId}" data-from-port="${conn.fromPort}" data-to-id="${conn.toId}" data-to-port="${conn.toPort}">×</button>` : ''}
                        </div>
                    </div>
                `;
			...
			pathSelects.forEach((el) => {
				el.addEventListener('click', () => {
					...
					if (conn) {
						...
						const targetInput =
							targetBlock && targetBlock.inputs[toPort]
								? window.WorkflowApp.Helpers.portName(targetBlock.inputs[toPort])
								: `input${toPort}`;
						const targetInputDesc =
							targetBlock && targetBlock.inputs[toPort] ? window.WorkflowApp.Helpers.portDesc(targetBlock.inputs[toPort]) : '';
