...
1. 修复了面板弹出后修改主题不跟随渲染的问题
2. 修复了自定义主题保存成功但是不生效的问题
3. 修复了修改快捷键不生效的问题
4. 新增了对于不支持的快捷键的提示
5. 补全部分未国际化的文案
6. 增加了测试用例生成方式的行为控制
...
1. AI 助手弹出模式：设置中新增"面板展开方式"选项，支持内置（编辑器内嵌）和弹出（独立浮动窗口）两种模式，通过 AI 开关按钮一键展开/收起
2. AI 助手弹出面板联动：弹出模式下 AI 开关按钮自动管理弹出窗口的创建与销毁，面板状态与模式选择实时联动
3. 主题同步机制：主窗口主题变更自动同步到弹出面板，保持界面风格统一

## 优化

1. 完善中英文双语映射：
    - 文件树右键菜单"删除 N 个项目"、"弹出预览"、"重命名"支持中英切换
    - 设置中 AI 助手选项卡、面板展开方式、主题同步等文案支持中英切换
    - 主题名称预设值内嵌中英双语映射
2. AI 助手初始化延迟加载，避免阻塞页面首屏渲染

## 修复

1. 修复项目右键菜单"Rename"未国际化的问题
2. 修复文件树批量删除和弹出预览硬编码英文文案的问题
